...
class MultiLLaMAForCausalLM(nn.Module):
    def __init__(self, lang_model_path, patch_size = 32, vision_encoder = 'ViT'):  
        ...
        self.embedding_layer = MyEmbedding(patch_size = patch_size, vision_encoder = vision_encoder)
    ...
    def generate(self, lang_x,vision_x):
        ...
        with torch.no_grad():
            ...
            generation = self.lang_model.generate(inputs_embeds = input_embedding,max_new_tokens =300,top_k=50)
            #generation = self.lang_model.generate(inputs_embeds = input_embedding)
        return generation
