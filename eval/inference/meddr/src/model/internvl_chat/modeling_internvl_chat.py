...
class InternVLChatModel(PreTrainedModel):
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config,
        ...
        num_images_in_question = question.count('<image>')
        if num_images_in_question != len(pixel_values):
            raise ValueError("Number of images in question does not match number of pixel_values provided")
        ...
        # image_tokens = ''.join([IMG_START_TOKEN + IMG_CONTEXT_TOKEN * self.num_image_token + IMG_END_TOKEN 
    ...
    def generate(
        ...
        if pixel_values is not None:
            pixel_values_batch = torch.cat(pixel_values, dim=0).cuda()

        # Extract visual features
        vit_embeds = self.extract_feature(pixel_values_batch)  # shape: (num_images, C)
        B, N, C = input_embeds.shape
